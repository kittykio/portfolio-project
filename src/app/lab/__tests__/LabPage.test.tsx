...
describe('LabPage', () => {
  ...
  it('auto-advances unless motion is reduced', () => {
    jest.useFakeTimers(); const { rerender } = render(<LabPage />); act(() => jest.advanceTimersByTime(4500)); expect(screen.getByText('Building Paperform')).toBeInTheDocument(); reduceMotion = true; rerender(<LabPage />); act(() => jest.advanceTimersByTime(9000)); expect(screen.getByText('Building Paperform')).toBeInTheDocument(); expect(screen.getByTestId('lab-hero')).toHaveAttribute('data-paused', 'true'); jest.useRealTimers();
  });
