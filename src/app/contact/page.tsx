...
export default function ContactPage() {
  ...
  return <SectionWrapper title={ja ? '相談する' : 'Let’s make something'} subtitle={ja ? 'プロダクト、インターフェース、クリエイティブなウェブ体験について。まずは必要なことを教えてください。' : 'For products, interfaces, and creative web experiences. Tell me what you need, and we can find a thoughtful way forward.'} className="mx-auto mt-24 w-full max-w-4xl px-4 pb-[42rem]">
    <form onSubmit={submit} className="grid gap-5 rounded-3xl bg-surface-glass p-6 shadow-[0_16px_40px_var(--shadow)] sm:grid-cols-2 sm:p-10">
      ...
      <div className="sm:col-span-2 flex flex-wrap items-center gap-4"><button type="submit" disabled={status === 'sending'} className="rounded-full bg-flame-500 px-6 py-3 font-bodyBold text-white transition hover:bg-flame-700 disabled:opacity-60 dark:bg-lemon dark:text-black">{status === 'sending' ? (ja ? '送信中…' : 'Sending…') : (ja ? '相談を送る' : 'Send inquiry')}</button>{status === 'sent' && <p className="font-bodyBold text-flame-500 dark:text-lemon" role="status">{ja ? '受け取りました。ありがとう！' : 'Received — thank you!'}</p>}{status === 'error' && <p className="text-flame-500" role="alert">{ja ? 'もう一度試してください。' : 'Please try again.'}</p>}</div>
    </form>
  </SectionWrapper>;
}
