...
export default function ShareButton({ title, url }: { title: string; url?: string }) {
  ...
  return <div className="flex items-center gap-2" aria-label="Share this page"><button type="button" onClick={share} className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-2 text-sm font-bodyBold hover:border-flame-500 hover:text-flame-500"><RiShareLine aria-hidden />Share</button><button type="button" onClick={copy} className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-2 text-sm font-bodyBold hover:border-flame-500 hover:text-flame-500"><RiFileCopyLine aria-hidden />{copied ? 'Copied' : 'Copy link'}</button><span className="sr-only" aria-live="polite">{copied ? 'Link copied to clipboard' : ''}</span></div>;
}
